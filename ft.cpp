...
vector<Funnel> FunnelTree(const TriangleMesh& mesh, const indexType s) {
...

    {
        ...
        list[0] = Funnel(p, q, p, facesAt_s, mesh.pistance2(s, p), mesh.pistance2(p, q), spq, psw, 0);
        ...
        for (indexType i = 1; i < n; i++) {
            ...
            list[i] = Funnel(p, q, p, facesAt_s, mesh.pistance2(s, p), mesh.pistance2(p, q), spq, psw, 0);
            vector<indexType> &sequence = list[i].sequence;
            swap(*find(sequence.begin(), sequence.end(), psqIndex), sequence[n - 1]);
        }
    }

    typedef unordered_map<Triangle, pair<size_t, double>, HashNComp> FunnelDict;
    ...
    for (size_t start = 0, end = n;;) {
        ...
        for (size_t i = start; i < curr_end; i++) {
            ...
            if (psv >= f.psw) {
                f.q = v; f.pq2 = pv2; f.spq = spv; f.psw = psw_new;
                goto find_v_suchthat_funnelhas2children;
            }

            const double pvs = angle(pv2, sv2, f.sp2), vsw = f.psw - psv, svq = pvq - pvs;
            #pragma omp atomic capture
            { f.childrenIndex = end; end += 2; }
            list[f.childrenIndex] = Funnel(f.p, v, f.x, f.sequence, f.sp2, pv2, spv, psw_new, f.topright_angle);
