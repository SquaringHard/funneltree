...
vector<Funnel*> FunnelTree(const TriangleMesh& mesh, const Point &s) {
    ...
    vector<Funnel*> list;
    typedef unordered_map<const Funnel*, const double, Hasher, Hasher> FunnelDict;
    ...
    while (curr < list.size()) {
        ...
        {   
            ...
            {
                // each task iterate 4 funnels
                #pragma omp taskloop reduction(push_back : local_list) num_tasks((list.size() - curr) >> 2)
                for (size_t i = curr; i < list.size(); ++i) {
                    ...
                    funnel->x = v; // to use for hash function
                    pair<FunnelDict::iterator, bool> temp;
                    #pragma omp critical
                    temp = twoChildrenFunnels.try_emplace(funnel, pvs);
                    if (temp.second) continue;
                    
                    /////////////////////////// CLIP OFF FUNNELS ////////////////////////////

                    const Funnel* oldFunnel = temp.first->first;
                    Funnel *const oldChildPV = oldFunnel->childPV, *const oldChildVQ = oldFunnel->childVQ;
                    const double sv2 = oldChildVQ->sp, pvs2 = temp.first->second;
